####################################################################################################
#   BNL UAS COGS example. Sourcing data from a Google Cloud Storage Bucket
#
#
#
...
example_kg_points <- rgdal::readOGR(file.path(dirname(here::here()),
...
# RGB
rgb_ras <- rast(paste0(rgb_cog.url,rgb_raster_name))
RGB(rgb_ras) <- c(1,2,3)
rgb_ras
...
# CHM
chm_ras <- rast(paste0(chm_cog.url,chm_raster_name))
...
# TIR
tir_ras <- rast(paste0(tir_cog.url,tir_raster_name))
